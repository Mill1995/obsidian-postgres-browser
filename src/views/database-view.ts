...
export class PostgresBrowserView extends ItemView {
  ...
  private buildLayout(container: HTMLElement): void {
    ...
    this.queryView = new QueryView(
      this.queryContainer,
      (queryText) => void this.onRunQuery(queryText),
    );
  ...
  private async loadSchemaTree(): Promise<void> {
    ...
    try {
      const sql = await this.plugin.connectionManager.getConnection(config);
      ...
      const tree = await this.plugin.schemaIntrospection.getSchemaTree(sql);
  ...
  private async onTableSelected(schema: string, table: string): Promise<void> {
    const config = this.getActiveConfig();
    if (!config) return;
    ...
    try {
      ...
      const [result, detail] = await Promise.all([
        ...
        this.plugin.schemaIntrospection.getTableDetail(sql, schema, table),
      ]);
      ...
      new Notice(`Preview failed: ${qErr.message || String(err)}`);
    }
  }

  private async loadSchemaDetail(schema: string, table: string): Promise<void> {
    const config = this.getActiveConfig();
    if (!config) return;
    ...
    try {
      const sql = await this.plugin.connectionManager.getConnection(config);
      const detail = await this.plugin.schemaIntrospection.getTableDetail(
        sql,
        schema,
        table,
      );
  ...
  private async onRunQuery(queryText: string): Promise<void> {
    ...
    try {
      ...
      const qErr = err as QueryError;
      this.queryView.showError(qErr.message ? qErr : { message: String(err) });
    } finally {
