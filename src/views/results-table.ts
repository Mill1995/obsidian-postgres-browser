...
export class ResultsTable {
  ...
  private pendingDeletions: Map<
    number,
    { tr: HTMLTableRowElement; row: Record<string, unknown> }
  > = new Map();
  ...
  renderResult(result: QueryResult, columnMeta?: ColumnInfo[]): void {
      ...
      for (const col of result.columns) {
        ...
        if (isEditable) {
          ...
          td.addEventListener("dblclick", () => {
            this.startEditing(td, col, row[col], rowIdx, row);
          });
        }
      }
    }
  ...
  private startEditing(
    ...
    const meta = this.editConfig.columnMeta.find((c) => c.name === columnName);
    if (!meta) return;
    ...
    const editValue = existingPending ? existingPending.newValue : currentValue;

    // Capture original state before any pending change was applied
    const oldValue = existingPending ? existingPending.oldValue : currentValue;
  ...
  private async batchSave(): Promise<void> {
    ...
    for (const [key, change] of entries) {
      try {
        await config.onCellUpdate(change.column, change.newValue, change.row);
        ...
        new Notice(`Failed to save ${change.column}: ${message}`);
      }
    }
