...
export default class PostgresBrowserPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.addCommand({
      ...
      callback: () => {
        void this.activateViewInWindow();
      },
    });
  ...
  async loadSettings(): Promise<void> {
    ...
    this.settings.queryTimeoutSeconds = Math.max(
      1,
      Math.min(
        300,
        Math.round(
    ...
    this.settings.previewRowLimit = Math.max(
      1,
      Math.min(
        MAX_PREVIEW_ROW_LIMIT,
        Math.round(
          this.settings.previewRowLimit || DEFAULT_SETTINGS.previewRowLimit,
        ),
      ),
    );
